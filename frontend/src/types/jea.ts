// JEA (Jordan Engineers Association) service types (Workstream 6
// extraction from types/index.ts).
//
// Every type in this file encodes a JEA-specific concept: engineering
// projects, engineer records, workflow schemas, applications, review
// decisions, certificates, dashboard tiles built off JEA counts.
//
// Workstream 7+ moves these into modules/jea-*/types/ once the module
// folder split lands. Until then this file is the JEA-side view.
...
/**
 * The mixed platform + JEA dashboard payload. `total_applications`,
 * `pending_review`, `under_review`, `approved_today`,
 * `certificates_issued` are JEA workflow / lifecycle counts;
 * `active_services` counts JEA service definitions; `total_users` is
 * platform-generic. Workstream 8 splits this into a platform base
 * (`PlatformDashboardStats { total_users }`) + a JEA extension
 * (`JeaDashboardStats extends PlatformDashboardStats { ... }`) once
