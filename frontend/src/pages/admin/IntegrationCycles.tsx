  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8" dir="rtl">
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">دورات التكامل مع Nashmi</h1>
          <p className="text-gray-500 text-sm mt-1">{cycles.length} دورة</p>
        </div>
        <div className="text-xs text-gray-400 bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 max-w-xs">
          <p className="font-medium text-gray-600 mb-1">نقاط التكامل</p>
      ...
      {cycles.length === 0 ? (
        <div className="text-center py-20 text-gray-400">
          ...
          <p className="font-medium">لا توجد دورات تكامل بعد</p>
          <p className="text-sm mt-1">
            ترسل Nashmi متطلباتها إلى{' '}
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex-shrink-0 text-xs text-gray-400 text-left space-y-1">
                    {cycle.requirements_received_at && (
                      <p>📥 {new Date(cycle.requirements_received_at).toLocaleDateString('ar-EG')}</p>
                    )}
                    {cycle.code_done_notified_at && (
                      <p>📤 {new Date(cycle.code_done_notified_at).toLocaleDateString('ar-EG')}</p>
                    )}
                    {cycle.feedback_received_at && (
                      <p>💬 {new Date(cycle.feedback_received_at).toLocaleDateString('ar-EG')}</p>
                    )}
                  </div>
                </div>
              </Link>
