...
- **2026-07-31** — P4-2 · Phantom governance files resolved. Created six stub documents cited by `BUILD_CONTRACT.md` that previously did not exist: `ARCHITECTURE.md`, `docs/SECURITY_CONTROLS.md`, `docs/BUSINESS_RULES_REGISTER.md`, `docs/adr/ADR-TEMPLATE.md`, `docs/adr/ADR-001-schema-driven-engine.md`, `RTM.md`. Each stub points at the current authoritative source (usually `docs/architecture/*` and the remediation ledger) and has a "STATUS: STUB — pending authorship" header. ADR-001 is authored retroactively since the schema-driven engine decision has been in force since the initial build. `BUILD_CONTRACT.md` §2 table gains a Status column marking each artifact AUTHORED vs STUB.
- **2026-07-31** — H-08 · Platform User/Organization JEA leaks extracted. `app/Models/User.php` no longer imports `Modules\JeaProjects\Models\{OfficeCoalition,OfficeCoalitionMember}`; the `activeCoalition()` accessor was lifted to `Modules\JeaProjects\Support\OfficeCoalitionResolver::activeCoalitionFor($user)`. `app/Models/Organization.php` no longer imports `Modules\JeaServices\Models\{Application,ServiceDefinition}` or `Modules\JeaProjects\Models\OfficeCoalition`; the unused `services()`, `applications()`, deprecated-shim `activeCoalition()` methods were removed (no in-repo callers). QuotaLedger's 2 call sites were switched to the new resolver. `BoundariesTest` PC_ALLOWLIST no longer needs entries for `Models/User.php` or `Models/Organization.php`. 858 pass, +5 OfficeCoalitionResolverTest.
